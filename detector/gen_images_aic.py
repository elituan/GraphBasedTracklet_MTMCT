...
def preprocess(src_root, dst_root, cfg):
    ...
    # Can be
    # sec_dir_list = ["train"] #['test', "train", "validation"]
    sec_dir_list = cfg.SEC_DIR
    dst_dir_list = [ dst_root + '/images/' + i for i in sec_dir_list]
    # dst_dir_list = [datasets/detection/images/test, (train, validation)]
    for i in dst_dir_list:
        if not os.path.isdir(i):
            os.makedirs(i)

    for i,x in enumerate(sec_dir_list):
        # x_path = AIC21-MTMC/datasets/AIC21_Track3_MTMC_Tracking/test
        x_path = src_root + '/' + x
        if os.path.isdir(x_path):
            for y in os.listdir(x_path):
                if y.startswith('S'):
                    # y_path = AIC21-MTMC/datasets/AIC21_Track3_MTMC_Tracking/test/S06
                    y_path = os.path.join(x_path,y)
                    for z in os.listdir(y_path):
                        # z_path = AIC21-MTMC/datasets/AIC21_Track3_MTMC_Tracking/test/S06/c041(042)
                        ...
                        if z.startswith('c'):
                            ...
                            # dst_img1_dir = datasets/detection/images/test/S06/c041/img1
